...
#
# Node-Install: Integrated Caplin BN + Standalone Lighthouse VC + Erigon EL + MEVboost
# Quickstart :: Go Clients :: Docker-free
#
...
valid_exec_clients = ['ERIGON-CAPLIN']
valid_validator_clients = ['LIGHTHOUSE']
...
if not args.network and not args.skip_prompts:
    ...
    index = SelectionMenu.get_selection(valid_networks,title='Node Install Quickstart :: CoinCashew.com',subtitle='Installs Integrated Erigon-Caplin EL-CL / Lighthouse VC / MEVboost\nSelect Ethereum network:')
...
if not VALIDATOR_ONLY:
    # Set clients to erigon
    execution_client = valid_exec_clients[0]
    execution_client = execution_client.lower()

# Set clients to lighthouse
validator_client = valid_validator_clients[0]
# Set to lowercase
validator_client = validator_client.lower()
...
def download_and_install_erigon():
    if execution_client == 'erigon-caplin':
        ...
        tar_filename = None
        for asset in assets:
           if asset['name'].endswith(f"{platform_arch.lower()}_{binary_arch.lower()}.tar.gz"):
               download_url = asset['browser_download_url']
        ...
        if eth_network=="ephemery":
            ...
            _network=f"--bootnodes={bootnodes}"
            subprocess.run(["sudo", "-u", "execution", "/usr/local/bin/erigon", "init", "--datadir=/var/lib/erigon", "/opt/ethpillar/testnet/genesis.json"])        
        else:
            _network=f"--chain={eth_network}"

        # Process sync barriers
        if eth_network=="mainnet" or eth_network=="sepolia" :
            _syncparameters='--prune.mode=full --experiment.persist.receipts.v2=false'
        else:
            _syncparameters=''

        if MEVBOOST_ENABLED:
            _mevparameters='--caplin.mev-relay-url=http://127.0.0.1:18550'
        else:
            _mevparameters=''            

        # Erigon with integrated Caplin CL client
        _caplin=f"--caplin.enable-upnp --caplin.discovery.addr=0.0.0.0 --caplin.discovery.port={CL_P2P_PORT} --caplin.discovery.tcpport={CL_P2P_PORT} --caplin.max-peer-count={CL_MAX_PEER_COUNT} --beacon.api.addr=0.0.0.0 --beacon.api.port={CL_REST_PORT} --beacon.api=beacon,validator,builder,config,debug,events,node,lighthouse --caplin.checkpoint-sync-url={sync_url}/eth/v2/debug/beacon/states/finalized"

        ##### ERIGON-CAPLIN-Integrated SERVICE FILE ###########
        erigon_service_file = f'''[Unit]
...
KillSignal=SIGINT
TimeoutStopSec=900
ExecStart=/usr/local/bin/erigon --datadir=/var/lib/erigon {_network} --port={EL_P2P_PORT} --torrent.port=42069 --http.port={EL_RPC_PORT} --maxpeers={EL_MAX_PEER_COUNT} --http.api=web3,eth,net,engine --metrics --pprof --prune.mode=minimal --authrpc.jwtsecret={JWTSECRET_PATH} {_syncparameters} {_caplin} {_mevparameters}

[Install]
WantedBy=multi-user.target
'''
        erigon_temp_file = 'execution_temp.service'
        global erigon_service_file_path
...
def download_lighthouse():
    if validator_client == 'lighthouse':
        # Change to the home folder
        os.chdir(os.path.expanduser("~"))
...
def install_lighthouse_validator():
    ...
    if validator_client == 'lighthouse' and VALIDATOR_ENABLED:
...
def finish_install():
    ...
    if execution_client == 'erigon-caplin':
        print(f'Erigon-Caplin Version: \n{erigon_version}\n')

    if validator_client == 'lighthouse' and NODE_ONLY == False:
    ...
    if not VALIDATOR_ONLY:
        answer=PromptUtils(Screen()).prompt_for_yes_or_no(f"\nInstallation successful!\nSyncing a Erigon/Caplin node for validator duties can be as quick as a few hours.\nWould you like to start syncing now?")
        if answer:
            os.system(f'sudo systemctl start execution')
            if MEVBOOST_ENABLED:
    ...
    if answer:
        if not VALIDATOR_ONLY:
            os.system(f'sudo systemctl enable execution')
        if VALIDATOR_ENABLED:
